...
#
#
# ## health and  water impacts for RD-slide
# png("plots/RD_combination.png", width = 12.5, height = 17, unit = "cm", res = 600)
# gridExtra::grid.arrange(plots[["HumanHealth" ]],
#                         plots[["Water Withdrawal"]],
# ncol= 1)
# dev.off()
#
...
# png("UBA/Resources2.png", width = 20.5, height =10, unit = "cm", res = 600)
# gridExtra::grid.arrange(plots[["Fossil depletion" ]],
#                         plots[["Mineral resource depletion"]],
#                         ncol= 2)
# dev.off()
...
lu_variants = inline.data.frame(
    'tech.variant;         new',
    'default mix        ;   default mix'  ,
    'TAX30_betr_rf      ; TAX30_wood_rf',
    'TAX30_begr_betr_rf ; TAX30_w&g_rf' ,
    'TAX30_begr_betr_ir ; TAX30_w&g_ir' ,
    'TAX5_betr_rf       ; TAX5_wood_rf',
    'TAX5_begr_betr_rf  ; TAX5_w&g_rf' ,
    'TAX5_begr_betr_ir  ; TAX5_w&g_ir' ,
    'TAX0_betr_rf       ; TAX0_wood_rf',
    'TAX0_begr_betr_rf  ; TAX0_w&g_rf' ,
    'TAX0_begr_betr_ir  ; TAX0_w&g_ir' ,
    'residue            ; residue'     ,
    NULL
)
...
for (i in c('Ecosystem|Natural land transformation',
    ...
    data.plot <- df.imp.all %>%
        ...
        select(model, scenario, technology, tech.variant, value) %>%
        factor.data.frame()
    ...
    data.plot <- data.plot %>%
        ...
        order.levels(
            ...
            tech.variant = c(
                ...
                'residue')) %>%
        replace_column(lu_variants, tech.variant, new) %>%
        factor.data.frame()
        ...
        data.plot2 <- data.plot %>%
            filter(!is.na(model)) %>%
            group_by(scenario, tech.variant, model,x) %>%
            summarize(value = sum(value)) %>%
            ungroup()

        data.plot1 <- data.plot2 %>%
            group_by(scenario, tech.variant, x) %>%
            calc_quantiles( probs = c(q0 = 0, q25 = 0.25, q30 = 0.3, q50 =
                                          0.5, q75 = 0.75, q70 = 0.7, q100 = 1)) %>%
            ungroup() %>%
            spread(quantile, value)



        data.plot = data.plot %>% filter(is.na(model))
        ...
        p <- ggplot() +
            ...
            scale_shape_manual(
                 values = c(
                     ...
                     'TAX30_wood_rf'      = 'circle',
                     'TAX30_w&g_rf' = 'square',
                     'TAX30_w&g_ir' = 'diamond',
                     'TAX5_wood_rf'       = 'circle open',
                     'TAX5_w&g_rf'  = 'square open',
                     'TAX5_w&g_ir'  = 'diamond open',
                     'TAX0_wood_rf'       = 'circle plus',
                     'TAX0_w&g_rf'  = 'square plus',
                     'TAX0_w&g_ir'  = 'diamond plus',
                     'residue'            = 'cross',
            ...
            geom_errorbar(
                ...
                mapping = aes(x = x,
                              ...
                              # upper  = q75,
                              ymax   = q100),
                width = 0.2, stat = "identity", color = "grey70", size = 0.3) +
            geom_point(
                data = data.plot2,
        ...
        ggsave(gsub('(\\|| )', '', paste0('plots/spread2_', i, '_bio.png')), p,
